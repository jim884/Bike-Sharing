...
trainOrig <- read.csv("..//train.csv") #use nrows=1000 rows for speed during feature engineering
#test <- read.csv("..//test.csv") #use nrows=1000 rows for speed during feature engineering

set.seed(6345)
smp_size <- floor(0.7 * nrow(trainOrig))

train_ind <- sample(seq_len(nrow(trainOrig)), size = smp_size)
train <- trainOrig[ train_ind,]
test  <- trainOrig[-train_ind,]
saved <- test$count
...
featureEngineer <- function(df) {
  ...
  #df$weekday <- as.factor(df$weekday, levels = c("lunes", "martes", "miercoles", "jueves", "viernes", "sabado", "domingo")) #order factors
...
rmsle(saved,as.numeric(test$count))
